...
function M.ensure_io_view()
  ...
  local granular_test_cases = cache.get_granular_test_cases(platform, contest_id, problem_id)
  local display_tests = granular_test_cases
    or cache.get_test_cases(platform, contest_id, problem_id)

  if display_tests and #display_tests > 0 then
    local input_lines = {}
    for _, tc in ipairs(display_tests) do
